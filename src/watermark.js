...
WaterMark.prototype.init = function (opts) {
  ...
  _opts.renderType = opts.renderType;

  /* 如果设置水印挂载的父元素的id */
  var containerBox = (_opts.containerBox =
    (opts.containerNode && document.getElementById(opts.containerNode)) ||
    document.body);
...
WaterMark.prototype._getPageWidthHeight = function (_opts, containerBox) {
  ...
  _opts.pageWidth = Math.max(
    containerBox.scrollWidth,
    containerBox.clientWidth
  );
  ...
  _opts.pageHeight = Math.max(
    containerBox.scrollHeight,
    containerBox.clientHeight
  );
};
...
WaterMark.prototype._caleMarkCount = function (_opts, opts) {
  ...
  _opts.cols = parseInt(
    (_opts.pageWidth - _opts.x) / (_opts.boxWidth + opts.xSpace)
  );
  _opts.xSpace = parseInt(
    (_opts.pageWidth - _opts.x - _opts.boxWidth * _opts.cols) / _opts.cols
  );

  /*三种情况下会重新计算水印行数和y方向水印间隔：1、水印行数设置为0，2、水印长度大于页面长度，3、水印长度小于于页面长度*/
  _opts.rows = parseInt(
    (_opts.pageHeight - _opts.y) / (_opts.boxHeight + opts.ySpace)
  );
  _opts.ySpace = parseInt(
    (_opts.pageHeight - _opts.y - _opts.boxHeight * _opts.rows) / _opts.rows
  );
};
...
WaterMark.prototype._calcTotalLength = function (_opts, opts) {
  _opts.allWidth =
    _opts.x + _opts.boxWidth * _opts.cols + _opts.xSpace * (_opts.cols - 1);
  _opts.allHeight =
    _opts.y + _opts.boxHeight * _opts.rows + _opts.ySpace * (_opts.rows - 1);
};
